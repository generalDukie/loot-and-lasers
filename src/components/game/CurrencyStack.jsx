  ...
  return (
    <div className="flex flex-col h-full gap-0.5">
      <div className="flex-1 flex items-center gap-1.5 px-2 rounded-lg bg-background/90 border border-border/50">
        <Fuel className={`${ic} shrink-0`} style={{ color: "#00D4FF" }} />
        <span className={`font-display font-bold ${txc} tabular-nums leading-none`} style={{ color: "#00D4FF" }}>{fuel.toLocaleString()}</span>
        <span className={`${lbc} text-muted-foreground uppercase tracking-wider hidden sm:inline leading-none`}>Fuel</span>
      </div>
      <div className="flex-1 flex items-center gap-1.5 px-2 rounded-lg bg-background/90 border border-border/50">
        <Sparkles className={`${ic} shrink-0`} style={{ color: "#C084FC" }} />
        <span className={`font-display font-bold ${txc} tabular-nums leading-none`} style={{ color: "#C084FC" }}>{stardust.toLocaleString()}</span>
        <span className={`${lbc} text-muted-foreground uppercase tracking-wider hidden sm:inline leading-none`}>Stardust</span>
      </div>
