...
test("Corporate Offices replaces Ship Hangar in live navigation", () => {
  ...
  assert.match(rulesGd, /should_show_manufacturer_badge/);
  assert.match(rulesGd, /COMPANY_BADGE_ICON/);
  assert.match(rulesGd, /\"crown\"/);
  ...
  assert.match(uiIcon, /\"leaderboard\": \"trophy\"/);
  assert.match(shop, /sell_tab/);
  assert.match(shop, /make_item_name_row/);
  assert.match(shell, /SCENE_NEXUS[\s\S]*\"satellite\"/);
  assert.match(shell, /SCENE_LEADERBOARD[\s\S]*\"trophy\"/);
