...
Four Companies: CNC, BJS, DTD, GORP. Each manufactures four of eight Gear slots; each slot has exactly two Companies; each Company has exactly one premium slot (Weapon or Ship Module). Each legal company×slot pair has three named variants (`COMPANY_GEAR_VARIANT_COUNT = 3`, catalog size 48). New Gear names come from that catalog with no company-token prefix; `visual_id` selects the SVG glyph. Ordinary generation picks the slot first, then 50/50 among the two legal Companies (`rollManufacturerForSlot`). Commission Gear uses the token's Company. Manufacturer is immutable after generation. Retired identities `TTT` and `RDR` have no live aliases.
...
Manual Shipment: exactly five owned, unequipped, eligible, same-Company Gear IDs. `ShipmentBaseValue = sum(persisted sell_value)`. `ShipmentPayout = roundHalfUp(base × 1.10)` (`SHIPMENT_PAYOUT_BPS = 11000`). +100 Company reputation. Company level = `floor(reputation / 1500) + COMPANY_STARTING_LEVEL` (`COMPANY_STARTING_LEVEL = 1`). Cumulative `shipment_count` is stored separately.

Token rarity: no token at the starting level. Awarded rank `rankIndex = L - COMPANY_STARTING_LEVEL`; Epic when `(rankIndex - 1) mod 4` equals the Company offset (CNC 0, BJS 1, DTD 2, GORP 3); otherwise Rare. First CNC token (level 2) stays Epic. One waiting token per Company, independently. A second token for that same Company creates persisted overflow without undoing the Shipment. Same-Company Shipments are blocked until the player spends one token via Commission creation. While overflow is pending, that Company cannot gain reputation from any source, including admin grants. Overflow on one Company never blocks another Company from earning or storing a token.
