    ...
    >
      ...
      <div className="flex items-center gap-1 shrink-0">
        {item.type === "consumable" && onUse && (
          <button onClick={() => onUse(item)} className="text-[10px] bg-accent/15 hover:bg-accent/25 text-accent px-1.5 py-0.5 rounded font-medium transition-colors">Use</button>
        )}
        {onEquip && !item.is_equipped && (
          <button onClick={() => onEquip(item)} className="text-[10px] bg-primary/10 hover:bg-primary/20 text-primary px-1.5 py-0.5 rounded font-medium transition-colors">Equip</button>
        )}
        {item.is_equipped && onEquip && (
          <button onClick={() => onEquip(item)} className="text-[10px] bg-muted hover:bg-muted/80 text-muted-foreground px-1.5 py-0.5 rounded font-medium transition-colors">Off</button>
        )}
        {onSell && !item.is_equipped && (
          <button onClick={() => onSell(item)} className="text-[10px] bg-destructive/10 hover:bg-destructive/20 text-destructive px-1.5 py-0.5 rounded font-medium transition-colors">✨{computeStardustValue(item)}</button>
        )}
      </div>
    </div>
  );
