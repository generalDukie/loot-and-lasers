...
export function useInventory(character, onCharacterChange) {
  ...
  const useConsumable = useCallback(async (item) => {
    if (!character || item.type !== "consumable" || !item.consumable) return { ok: false };
    const now = Date.now();
    const durationMs = (item.consumable.duration_hours || 6) * 3600 * 1000;
    const maxExpiry = now + durationMs * MAX_BUFF_STACKS;
    // One active buff per stat. The same stim again only extends its timer
    // (the stat effect never increases); a different tier of the same stat
    // overrides it with the new stim's mult and a fresh timer.
    const source = buffsRef.current ?? character.active_buffs ?? [];
    ...
    // A new stat stim is blocked once 3 distinct stats are already boosted.
    ...
    // Same stim already stacked to the 3× cap — can't extend, don't consume it.
    ...
    // A weaker stim must not overwrite a stronger active stim of the same stat.
    ...
    if (sameStatIdx >= 0) {
      ...
      if (existing.name === item.name) {
        ...
        buffs[sameStatIdx] = { stat: item.consumable.stat, mult: item.consumable.mult, expires_at: new Date(now + durationMs).toISOString(), name: item.name };
      }
    } else {
      buffs = [...active, { stat: item.consumable.stat, mult: item.consumable.mult, expires_at: new Date(now + durationMs).toISOString(), name: item.name }];
    }
    // Update synchronously so a rapid follow-up use sees the new buff state
    // before React re-renders (prevents blowing past the 3-stat / 3-stack caps).
    buffsRef.current = buffs;
    await api.entities.Character.update(character.id, { active_buffs: buffs });
    await api.entities.Item.delete(item.id);
    onCharacterChange?.({ active_buffs: buffs });
    await load();
    return { ok: true };
  }, [character, load, onCharacterChange]);
