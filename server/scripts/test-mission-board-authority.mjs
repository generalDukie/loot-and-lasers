...
await testAsync("Manual reroll regenerates the board", async () => {
  const { user } = makeCharacter();
  const first = await GetMissionBoard(user, {});
  ...
  assert.notDeepEqual(
    ...
    first.body.offers.map((o) => o.offer_id),
    "reroll produces new offer ids"
  );
});

await testAsync("Launch by offer_id uses server duration/efficiency + consumes board", async () => {
  const { user, ch } = makeCharacter();
  ...
  assert.equal(live.mission_board, null, "board consumed on launch");
