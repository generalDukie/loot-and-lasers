...
## XP, Stardust, efficiency, rarity/drop preview) are now owned by Node. Godot requests
## the board via `MissionManager.ensure_board()` → `GetMissionBoard`, then renders the
## authoritative values the server returns.
##
