...
/** Hub stage content — fills the shell main column (nav/chrome come from GameLayout). */
export default function SpaceStationHub({ children }) {
  const { user } = useAuth();
  ...
  return (
    <div className="relative flex-1 min-h-0 w-full flex flex-col overflow-hidden">
      {/* Station art — z-0 (not negative) so it stays above the shell surface */}
      ...
      <div className="relative z-[1] flex-1 min-h-0 overflow-y-auto overflow-x-hidden flex flex-col">
        <div
