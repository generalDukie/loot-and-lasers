...
Token rarity: one-indexed level `L`, Epic when `(L-1) mod 4` equals the Company offset (CNC 0, BJS 1, DTD 2, GORP 3); otherwise Rare. One waiting token per Company. A second token creates persisted overflow without undoing the Shipment. Same-Company Shipments are blocked until the player spends one token via Commission creation.
