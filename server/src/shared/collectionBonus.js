...
import { dungeonBadgeCount } from "../../../src/lib/dungeonBadges.js";
import { catalogGearDiscoveryCount } from "../../../src/lib/productionMath/companies.js";
...
export function getCollectionPercentage(character, gearTotal) {
  ...
  const gearTotalN = Math.max(0, Math.floor(Number(gearTotal) || 0));
  const gear = gearTotalN > 0
    ? catalogGearDiscoveryCount(character?.discovered_gear)
    : (character?.discovered_gear || []).length;
