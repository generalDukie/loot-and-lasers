  ...
  return (
    <header className="sticky top-0 z-50 bg-background/80 border-b border-border/30 backdrop-blur-sm shrink-0">
      <div
        ...
        style={{
          padding: "clamp(0.4rem, 0.7cqi, 0.85rem) clamp(0.75rem, 1.8cqi, 2rem)",
        }}
      >
          ...
          {onHub ? (
            ...
            <Link to="/" className="pointer-events-auto focus:outline-none flex flex-col items-center leading-none relative group">
              ...
              <motion.div
                ...
                style={{ right: "clamp(-2.4rem, -2.2cqi, -1.6rem)" }}
              >
                <CornerUpLeft style={{ width: "clamp(0.85rem, 1.1cqi, 1.15rem)", height: "clamp(0.85rem, 1.1cqi, 1.15rem)" }} />
                <span
                  className="font-display tracking-[0.2em] uppercase mt-0.5 whitespace-nowrap text-white"
                  style={{ fontSize: "clamp(0.45rem, 0.65cqi, 0.6rem)" }}
                >
                  Hub
                </span>
              </motion.div>
        ...
        <div
          ...
          style={{ gap: "clamp(0.25rem, 0.6cqi, 0.6rem)" }}
        >
...
function BrandMark({ interactive = false }) {
  const size = { fontSize: "clamp(1.05rem, 2.1cqi, 2.15rem)" };
  const base =
...
function HeaderIconLink({ to, title, children }) {
  return (
    <Link
      ...
      style={{ padding: "clamp(0.4rem, 0.55cqi, 0.6rem)", fontSize: "clamp(0.95rem, 1.15cqi, 1.2rem)" }}
    >
      {children}
    </Link>
  );
